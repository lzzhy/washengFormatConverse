...
function renderMenu(menu: Menu, isHasSubmenu: boolean) {
  return (
    <ElMenuItem index={menu.path} key={menu.path} class={[!isHasSubmenu ? "main-menu" : "sub-menu", appStore.isHoverAdminLayoutSide ? "hover" : ""]}>
      {menu.icon ? (
        <div>
          <Icon name={menu.icon as any}></Icon>
        </div>
      ) : null}
      <span class={appStore.isHoverAdminLayoutSide ? "hover" : ""}>{menu.title}</span>
    </ElMenuItem>
  );
}
...
function renderSubMenu(menu: Menu) {
  ...
  return (
    <ElSubMenu index={menu.name} key={menu.path} class='main-menu'>
      {{
        title: () => (
          <>
            <div>
